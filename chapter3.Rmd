...
Cross tabulations for the chosen variables vs. High Use of Alcohol. Comments after the tables.
...
ROC curve can be seen above. AUC (=Area Under Curve) for the model (and predictions) is $0.70$, which is I think better than I anticipated. For random data (or uninformative classifier) the AUC should be $0.5$ (denoted but the diagonal line). Perfect model would have AUC value of $1$. Well, the model is better than random values.
